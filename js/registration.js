        ...
        personnummerInput.addEventListener('keydown', function(e) {
            if (e.key >= '0' && e.key <= '9') {
                ...
                if (pos < mask.length && mask[pos] !== '-') {
                    let newValue = personnummerInput.value.substring(0, pos) + e.key + personnummerInput.value.substring(pos + 1);
                ...
                if (pos > 0) {
                    ...
                    pos--;
                    let char = mask[pos];
                    let newValue = personnummerInput.value.substring(0, pos) + char + personnummerInput.value.substring(pos + 1);
                    personnummerInput.value = newValue;
                    personnummerInput.setSelectionRange(pos, pos);
                }
            } else {
